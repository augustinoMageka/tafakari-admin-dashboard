...
import { updateDBDoc } from "@/services/dbServices";
...
export default function EditCategoryModal({
  ...
  const handleEdit = async (values) => {
    ...
    const modifiedCategories = categories.map((cat) => {
      if (cat.name == oldCategory.name) {
        // console.log("reaching..");
        return { ...oldCategory, name: values.name };
      } else return cat;
    });
  ...
  useEffect(() => {
    if (oldCategory) {
      setInitialCategory(oldCategory.name);
    }
  }, [oldCategory.name]);
